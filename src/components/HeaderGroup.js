...
export default function HeaderGroup() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      {/* TOP BAR */}
      <div className={`w-full bg-gradient-to-r from-rose-700 to-rose-600 text-white transition-all duration-500 ${scrolled ? "max-h-0 opacity-0" : "max-h-12 py-2.5"}`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between text-xs font-semibold">
          <div className="flex items-center gap-2"><FaMapMarkerAlt /><span>522/3 Hindusthan Hospital Road, Coimbatore</span></div>
          <div className="flex gap-4">
            <div className="flex gap-2"><FaPhoneAlt /><span>+91 422 432 7777</span></div>
            <div className="flex gap-2"><FaEnvelope /><span>info@hindusthan.com</span></div>
          </div>
        </div>
      </div>
      ...
      <div className="w-full flex justify-center mt-4">
        <div className="bg-white w-[95%] max-w-7xl rounded-full shadow-xl px-6 py-3 flex justify-between items-center">
          ...
          <nav className="hidden lg:flex gap-6">
            {nav.map((item, i) => (
              <div key={i} className="relative" onMouseEnter={() => openDropdown(i)} onMouseLeave={closeDropdown}>
                ...
                <AnimatePresence>
                  {item.children && activeDropdown === i && (
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                      className={`absolute top-full mt-3 bg-white rounded-xl shadow-xl p-2 z-50 ${item.mega ? "w-[900px] grid grid-cols-4" : "w-64"}`}>
                      {item.children.map((sub, j) =>
                        typeof sub === "string"
                          ? <NavLink key={j} to={toSlug(sub)} className="block px-4 py-2 text-sm hover:text-rose-600">{sub}</NavLink>
                          : <NavLink key={j} to={sub.to} className="block px-4 py-2 text-sm hover:text-rose-600">{sub.label}</NavLink>
                      )}
                    </motion.div>
                  )}
                </AnimatePresence>
          ...
          <div className="hidden lg:flex items-center gap-3">
            ...
            <button className="bg-rose-600 text-white px-5 py-2 rounded-full text-xs font-bold">APPOINTMENT</button>
          </div>

          {/* MOBILE RIGHT */}
          <div className="flex items-center gap-3 lg:hidden">
            <img src={certLogo} alt="NABH" className="w-8" />
            <button className="text-2xl text-rose-700" onClick={() => setMenuOpen(!menuOpen)}>
            ...
            {nav.map((item, i) => (
              <div key={i} className="border-b">
                <div className="flex justify-between items-center px-4 py-3 font-semibold"
                  onClick={() => item.children && setMobileDropdown(mobileDropdown === i ? null : i)}>
                ...
                {item.children && mobileDropdown === i && (
                  <div className="bg-gray-50">
                    {item.children.map((sub, j) =>
                      typeof sub === "string"
                        ? <NavLink key={j} to={toSlug(sub)} className="block px-6 py-2 text-sm">{sub}</NavLink>
                        : <NavLink key={j} to={sub.to} className="block px-6 py-2 text-sm">{sub.label}</NavLink>
                    )}
                  </div>
                )}
              </div>
